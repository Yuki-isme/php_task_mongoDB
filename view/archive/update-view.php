<div class="card-body">
    <table class="table table-hover table-bordered" id="billTable">
        ...
        <tbody>
            <?php if ($data['archives']) : ?>
                <?php foreach ($data['archives'] as $archive) : ?>
                    <tr>
                        <th scope="row"><input type="checkbox" class="check" value="<?= $archive->_id ?>"></th>
                        <td>
                            <select class="form-control" id="update-account" data-id="<?= $archive->_id ?>">
                                <?php foreach ($accountNames as $accountName) : ?>
                                    <option <?= $accountName ==  $archive->account ? 'selected' : '' ?>><?= $accountName ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td><?php echo $archive->bill_id ?></td>
                        <td>
                            <select class="form-control" id="update-service" data-id="<?= $archive->_id ?>">
                                <?php foreach ($serviceNames as $serviceName) : ?>
                                    <option <?= $serviceName ==  $archive->service ? 'selected' : '' ?>><?= $serviceName ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td><?php echo $archive->amount ?></td>
                        <td>
                            <select class="form-control" id="update-status" data-id="<?= $archive->_id ?>">
                                <?php foreach ($statusNames as $statusName) : ?>
                                    <option <?= $statusName ==  $archive->status ? 'selected' : '' ?>><?= $statusName ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td>
                            <select class="form-control" id="update-category" data-id="<?= $archive->_id ?>">
                                <?php foreach ($categoryNames as $categoryName) : ?>
                                    <option <?= $categoryName ==  $archive->category ? 'selected' : '' ?>><?= $categoryName ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        ...
                        <td>
                            <a href="/archive/edit/<?= $archive->_id ?>" class="btn btn-outline-primary">
                                <i class="fa-solid fa-pen"></i>
                            </a>
                            <button data-id="<?= $archive->_id ?>" class="btn btn-outline-danger delete">
...
<div class="card-footer">
    <nav aria-label="Page navigation">
        <ul class="pagination float-end" id="pagination">
            <?php if (count($data['archives']) > 0 && $data['page'] > 1) : ?>
                <li class="page-item">
                    <button class="page-link" data-page="<?= $data['page'] - 1 ?>" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </button>
                </li>
            <?php endif; ?>
            <?php for ($i = 1; $i <= $data['totalPage']; $i++) : ?>
                <li class="page-item <?= $data['page'] == $i ? 'active' : '' ?>"><button class="page-link" data-page="<?= $i ?>"><?= $i ?></button></li>
            <?php endfor; ?>
            <?php if (count($data['archives']) > 0 && $data['page'] < $data['totalPage']) : ?>
                <li class="page-item">
                    <button class="page-link" data-page="<?= $data['page'] + 1 ?>" aria-label="Next">
